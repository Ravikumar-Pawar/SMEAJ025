...
public class AccountHolderDetails extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AccountHolderDetails frame = new AccountHolderDetails();
	...
	public AccHolder_Details() {
		try {
           ...
           String[][] data=new String[200][5];
			...
			{ 
				data[j][0]=rs.getString("username");
			   data[j][1]=rs.getString("phno");
			   data[j][2]=rs.getString("city");
			   data[j][3]=rs.getString("amount");
		...
		contentPane.setLayout(null);
		   String[] column= {"NAME","PHONE","BRANCH","CURRENT BALANCE"};
		   String[][] data1=new String[j][4];
		    for(int i=0;i<j;i++)
		    {
		    	data1[i][0]=data[i][0];
		    	data1[i][1]=data[i][1];
		...
		JLabel lblNewLabel = new JLabel("ACCOUNT HOLDER DETAILS");
